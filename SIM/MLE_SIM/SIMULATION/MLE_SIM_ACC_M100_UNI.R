...
#   MODEL EVALUATION (MLE_SIM_ACC_M100_UNI)
...
sim <- "MLE_SIM_ACC_M100_UNI"             # simulation name
...
prob[["study_accuracy"]] <- 
  CJ(load.folder = load.dir,
     ...
     M = 100,
     M.start = c(1, 26), 
     M.probs = "uniform", #c("learn.theta", "learn.theta.neg", "uniform"),
     M.seed = 1337,
     eval.n = c(100,200,400,800,8000),
     eval.first = 1,
     eval.rdm = FALSE,
     select.method = c("rank", "se", "oracle"), 
     select.args = c("r=1", "c=1", "train", "learn"),
     select.limit = "sqrt",
     known.delta = c(FALSE),
