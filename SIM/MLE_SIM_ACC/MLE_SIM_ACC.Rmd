...
# TODO: remove methods not needed
...
info$pch <- c(rep(17, 2), rep(16,2), rep(18,4))
...
read_data <- function(sim, path=config$path, sep="\\"){
  dplyr::bind_rows(lapply(sim, function(x) readr::read_csv(file.path(path, paste0(x, ".csv"), fsep=sep), col_types = cols())))
}
...
preproc_data <- function(data, size=config$subset.size, seed=1337,
  ...
  out$raw <- data %>%
    ...
    mutate(abs.dev = final.acc.thetahat-final.learn.theta,
           rel.dev = abs.dev/final.learn.theta,
           abs.dev.c = final.acc.thetatilde-final.learn.theta,
           rel.dev.c = abs.dev.c/final.learn.theta,
  ...
  if(paired){
    ...
    dplyr::select(load.id, M.start, M, n.eval, n.learn, scenario, methods,
                  select.rule, infer.method, final.learn.theta, opt.theta) %>%
    dplyr::filter(select.rule %in% c("within1SE", "default")) %>%
    tidyr::spread(select.rule, final.learn.theta)
  }
  ...
  if(test){
    out$test <- out$raw %>% 
      SEPM.SIM::postproc_mle(analysis="acc") 
  }
...
# TODO: name final.mod.thetabar -> final.acc.thetabar
```
...
data$raw %>%
  filter(M==200) %>%
  ggplot(aes(select.rule, (final.learn.theta-opt.theta), colour=select.rule)) +
...
data$raw %>%
  filter(M==200) %>%
  ggplot(aes(select.rule, (final.learn.theta-opt.theta)/opt.theta, colour=select.rule)) +
...
- SEPM.SIM (Statistical Evaluation of Prediction Models - Machine Learning and Evaluation, R package): https://github.com/maxwestphal/SEPM.SIM

## Version History

- 13/05/2019 - Version 1: Initial release
- 16/05/2019 - Version 2: Added 'selection on based on model complexity'
- 12/12/2021 - Version 3: Reproduced original report with new package versions
